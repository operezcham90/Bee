...
IplImage       *Im1 = NULL;
IplImage       *Im2 = NULL;
CvMat          *descriptor_izq;
CvMat          *descriptor_der;
CvMat          *Mrotacion_izq;
CvMat          *Mrotacion_der;
CvMat          *Vrotacion_izq;
CvMat          *Vrotacion_der;
...
vector < double >pts3d_abejas;
vector < int   >proyec_izq;
vector < int   >proyec_der;
...
main(int argc, char **argv)
{
    ...
    char           *inicio =
	argc == 2 ? argv[1] : (char *) "./archivos.ini";

    ifstream        in(inicio);
    string          cadena;

    if (!in) {
	cerr << "Error al leer archivo de parametros de calibracion" <<
	    endl;
	return 0;
    }
    ...
    if (!fpr.is_open()) {
	cerr << "El archivo " << nombreParam <<
	    " NO existe, operacion abortada" << endl;
	return 0;
    }
...
-------------------------------------------------------------------------*/
    ...
    if (!fpr.is_open()) {
	cout << "El archivo NO existe, operacion abortada" << endl;
	return 0;
    }
...
-------------------------------------------------------------------------*/
    ...
    cvReleaseMat(&descriptor_izq);
    cvReleaseMat(&descriptor_der);
...
/**********************    E T A P A   D E  E X P L O R A C I O N *************************/
	...
	for (gen_no = 1; gen_no <= max_gen; gen_no++) {
	    ...
	    // GENERATION OF NEW POPULATION through SELECTION, XOVER ,
	    // MUTATION & RANDOM
	    // and PROJECTION into the images
	    generate_new_pop();

	    for (int i = 0; i < pop_size_off; i++)	// evaluación
							// nueva
							// población
	    {
		fitness_function_exp(&newpop[i], maxX, maxY, buf_F1,
				     buf_F2);
	    }

	    /*
	     * for (int i=0; i<pop_size_off; i++) cout <<
	     * newpop[i].xreal[0] << " " << newpop[i].xreal[1] << " " <<
	     * newpop[i].xreal[2] << "\t" << newpop[i].obj << "\t" <<
	     * newpop[i].u1 << " " << newpop[i].v1 << " " << newpop[i].u2
	     * << " " << newpop[i].v2 << "\t" << newpop[i].type << "\t"
	     * << newpop[i].nicho << endl; getchar();
	     */
...
opencv_abejas::input_param_file(string archivo)
{
   ...
   **************************************************************/
    if (!inI.is_open()) {
	// free(config_param);
	cout << "El archivo " << archivo << "NO existe, operacion abortada"
	    << endl;
	return 0;
    }
    parametros = archivo;
	...
	cerr <<
	    "\n Increase the value of MAXPOPSIZE in program  and re-run the program"
	    << endl;
	exit(-1);
    }
	...
	for (k = 0; k < nvar_real; k++) {
	    ...
	    cout << "\nxreal " << xreal_lower[k] << "," << xreal_upper[k]
		<< endl;
	}
    if (nvar_real > 0) {
	inI >> ans;
	...
	cout <<
	    "Error al elegir el tipo de gradiente. Funcion abortada. Revise archivo de parametros"
	    << endl;
	return 0;
    }
    string
	...
	cout <<
	    "Error al elegir el tipo de correlacion. Funcion abortada. Revise archivo de parametros"
	    << endl;
	return 0;
    }
    inI >> num_abejas;
	...
	cout << "Numero de abejas que se imprimiran del total: " <<
	    num_abejas << endl;
    else {
	num_abejas = pop_size;
	cout <<
	    "WARNING: Numero de abejas mayor al de la poblacion inicial" <<
	    endl;
    }
    inI >> rate_cross_rec;
			      ...
			      IplImage * buf_F2)
{
	...
	for (int k = 0; k < pop_total; k++) {
		...
		oldpop[k].xreal[j] =
		    (double) (cvRandReal(&rng) *
			      (xreal_upper[j] - xreal_lower[j]) +
			      xreal_lower[j]);
	    }
	}
    } else
 ...
 * **********************************************************/
void
opencv_abejas::proyecta(int popu, int op)
{
    ...
    cvProjectPoints2(X, Vrotacion_izq, Vtraslacion_izq, MIntrinsecos_izq,
		     NULL, x1);
    if (tipo_matriz == 0) {
	cvProjectPoints2(X, Vrotacion_der, Vtraslacion_der,
			 MIntrinsecos_der, NULL, x2);
    } else {
...
opencv_abejas::verifica_pop()
{
	...
	// cout<<"Advertencia: La Poblacion cambiara para cumplir
	// proporcion de operadores evolutivos, de: "<<pop_size_off<<" a:
...
====================================================================*/
void
opencv_abejas::generate_new_pop()
{
    int             k = 0,
	mate1,
	mate2,
    ...
    double          temp1,
                    temp2;
    preselect_tour();		// reinicia el orden de los individuos
				// para escoger en el torneo
	...
	if (mut_type)
	    ...
	    cerr <<
		"Error en archivo de parametros: Operador de mutacion no valido.  Abort";
	    exit(-1);
	}
	newpop[k].type = 1;
	k++;
	...
	if (cross_type)		// crossover SBX
	    ...
	    cerr <<
		"Error en archivo de parametros: Operador de cruzamiento no valido.  Abort";
	    exit(-1);
	}
	newpop[k].type = -1;
	newpop[k + 1].type = -1;
...
opencv_abejas::sh(CvRect r)
{
    ...
    for (int i = 0; i < pop_size + pop_size_off; i++) {
	// if(/*tempop[i].obj - alpha_share*(sum_izq + abs(sum_izq -
	// sum_der)) <= 0) // && */tempop[i].obj >= 0)
	{
	    // cout<<"mayor"<<tempop[i].obj<<"= "<<sum_izq<<" ,
	    // "<<sum_der<<" ALPHA"<<alpha_share;getchar();//tempop[i].obj 
	    // = 0.0;
	    tempop[i].obj -=
		alpha_share * (sum_izq + fabs(sum_izq - sum_der));
...
double
opencv_abejas::sh(int uno, int dos)
{
...
opencv_abejas::distanc(int one, int two, int op)
{
	...
	for (k = 0; k < nvar_real; k++)
	    sum +=
		square((oldpop[one].xreal[k] -
			oldpop[two].xreal[k]) / (xreal_upper[k] -
						 xreal_lower[k]));
	...
	for (k = 0; k < nvar_real; k++)
	    sum +=
		square((newpop[one].xreal[k] -
			newpop[two].xreal[k]) / (xreal_upper[k] -
						 xreal_lower[k]));
	result = sqrt(sum / nvar_real);
	break;
    case 3:
